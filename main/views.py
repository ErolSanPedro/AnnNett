...
def penaltyModule(info, b):
	...
	for var in PENALTY_TABLE:
		if str(b.ipaddress) == str(var.id_blacklist) and (b.ipaddress == info.dest_IP) and (var.status != 'blocked') and (var not in UPDATE_LIST): 
	...
	for var in PENALTY_TABLE:  #id_blacklist here is an IP address
		if str(b.ipaddress) == str(var.id_blacklist) and str(b.ipaddress) == str(info.dest_IP):
			isInList = 1
